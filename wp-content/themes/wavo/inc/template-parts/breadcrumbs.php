...
if ( ! function_exists( 'wavo_breadcrumbs' ) ) {
    ...
    function wavo_breadcrumbs( $args = array() ) {

        $breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );

        if ( ! is_object( $breadcrumb ) )
        $breadcrumb = new Wavo_Breadcrumb( $args );
    ...
    public function trail() {
        ...
        if ( 0 < $item_count ) {
            ...
            foreach ( $this->items as $item ) {
                ...
                $item = ! empty( $matches ) ? sprintf( '%s<span class="breadcrumb_link_text">%s</span>%s', $matches[1], $matches[2], $matches[3] ) : sprintf( '<span  class="breadcrumb_active">%s</span>', $item );

                // Wrap the item with its itemprop.

                // Add list item classes.
                $item_class = 'trail-item breadcrumb_item breadcrumb_link';
                ...
                if ( $item_count > 0 && $item_count !== $item_position ) {
                    $seperator = '<span class="breadcrumb_link_seperator"> / </span>';
                }
            ...
            $breadcrumb = sprintf(
                '<%1$s class="breadcrumb-trail">%3$s%4$s%5$s</%1$s>',
